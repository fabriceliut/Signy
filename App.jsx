...
const App = () => {
  ...
  const generateSignatureHTML = (isForCopy = false) => {
    ...
    const finalLogo = logoUrl.trim() !== '' ? logoUrl : `https://ui-avatars.com/api/?name=${name.split(' ').map(n=>n[0]).join('')}&background=fef3c7&color=92400e&bold=true&size=128`;
    const contactParts = [];
  ...
  return (
    <div className="min-h-screen bg-[#fafaf9] text-slate-900 font-sans selection:bg-amber-100 flex flex-col items-center">
      <div className="fixed inset-0 pointer-events-none overflow-hidden -z-10">
        <div className="absolute top-[-5%] left-[-5%] w-[100%] md:w-[40%] h-[40%] bg-amber-100/50 rounded-full blur-[100px]" />
        <div className="absolute bottom-[-5%] right-[-5%] w-[100%] md:w-[35%] h-[35%] bg-indigo-50 rounded-full blur-[100px]" />
      </div>
          ...
          <section className="lg:sticky lg:top-8 space-y-6 order-2">
            ...
            <div className="p-8 bg-slate-900 text-white rounded-[2.5rem] shadow-xl relative overflow-hidden group">
               ...
               <ul className="text-[11px] text-slate-400 space-y-3 leading-relaxed relative z-10">
                 <li className="flex gap-2"><span>&bull;</span> Remplissez vos informations à gauche.</li>
                 <li className="flex gap-2"><span>&bull;</span> Cliquez sur "Copier pour Gmail".</li>
                 <li className="flex gap-2"><span>&bull;</span> Collez directement dans vos réglages mail.</li>
               </ul>
            </div>
          </section>
        </main>
...
const InputField = ({ label, name, value, onChange, icon, placeholder }) => (
  <div className="space-y-2 group flex-1">
    ...
    <div className="relative flex items-center">
      ...
      <input type="text" name={name} value={value} onChange={onChange} className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 pl-12 pr-4 focus:outline-none focus:ring-2 focus:ring-slate-100 focus:bg-white transition-all text-sm text-slate-700" placeholder={placeholder} />
    </div>
  </div>
);
